...
#error cases
```{r}
a <- drc::drm(Response~Dose, data = drc_error_1, fct = LL.4())
```

```{r}
a <- dr4pl(Response~Dose, data = drc_error_1, method.robust = "Tukey")
plot(a, text.title = "Error plot #1", indices.outlier = 102)
```

```{r}
a <- drc::drm(Response~Dose, data = drc_error_2, fct = LL.4())
```
...
```
```{r}
a <- drc::drm(Response~Dose, data = drc_error_3, fct = LL.4())
```
...
```
```{r}
a <- drc::drm(Response~Dose, data = drc_error_4, fct = LL.4())
```
