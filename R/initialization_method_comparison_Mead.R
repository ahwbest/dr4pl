...
CompareInitializations <- function(data.to.comp,
  ...
  if(length(var.ref) == 0) {
    ...
    parm.drra <- rbind(coef(obj.drra.logistic), coef(obj.drra.Mead))

    result <- rbind(result,
                    cbind(parm.drra,
                          c(obj.drra.logistic$error.value, obj.drra.Mead$error.value)))

    row.names(result) <- c("drc", "drra/logistic", "drra/Mead")
...
#
setwd("C:\\Users\\Hyowon\\Research\\Dose_response_modelling\\Result")
sink(file = "parameter_comparison_Mead.txt")
