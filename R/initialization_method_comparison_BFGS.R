###
### Compare parameter estimates of drc and drra when the `BFGS' method is applied.
### Author: Hyowon An
### Last update: 12/30/2016
###

# --------------------------------------------------------------------------------
### Load libraries
#
library(drc)

# --------------------------------------------------------------------------------
### User-defined functions
#
ComparisonPlot <- function(obj.drc, obj.drra) {

  par.drc <- coef(obj.drc)
  par.drc <- par.drc[c(3, 4, 1, 2)]
  par.drc[3] <- -par.drc[3]
  names(par.drc) <- c("Lower limit", "IC50", "Slope", "Upper limit")

  # Make a scatter plot of a `drra' object
  #
  # Args:
  #   object: A `drra' object
  a <- ggplot(aes(x = Dose, y = Response), data = obj.drra$data)

  a <- a + stat_function(fun = DoseResponseCurve,
                         args = list(theta = par.drc),
                         colour = "blue",
                         size = 1.2)
  a <- a + stat_function(fun = DoseResponseCurve,
                         args = list(theta = obj.drra$parameters),
                         colour = "red",
                         size = 1.2)

  a <- a + geom_point(size = I(5), alpha = I(0.8))

  a <- a + labs(title = "Dose response curve")

  # Set parameters for the grids
  a <- a + theme(strip.text.x = element_text(size = 16))
  a <- a + theme(panel.grid.minor = element_blank())
  a <- a + theme(panel.grid.major = element_blank())
  a <- a + scale_x_log10()
  a <- a + theme_bw()

  # Set parameters for the titles and text / margin(top, right, bottom, left)
  a <- a + theme(plot.title = element_text(size = 20, margin = margin(0, 0, 10, 0)))
  a <- a + theme(axis.title.x = element_text(size = 16, margin = margin(15, 0, 0, 0)))
  a <- a + theme(axis.title.y = element_text(size = 16, margin = margin(0, 15, 0, 0)))
  a <- a + theme(axis.text.x = element_text(size = 16))
  a <- a + theme(axis.text.y = element_text(size = 16))

  plot(a)
}

CompareInitializations <- function(data.to.comp,
                                   var.dose,
                                   var.response,
                                   var.ref = NULL) {
  # Compare different initialization methods
  #
  # Args:
  #   data.to.comp: data to fit both drm and drra
  #   var.dose: variable corresponding to dose
  #   var.response: variable corresponding to response
  #   var.ref: reference variable
  #
  # Returns:
  #   Print the estimated parameters to standard output
  data.to.comp <- na.omit(data.to.comp)

  result <- c()

  # When there is only one drug.
  if(length(var.ref) == 0) {

    data.new <- subset(x = data.to.comp, select = c(var.dose, var.response))
    colnames(data.new) <- c("Dose", "Response")
    n <- nrow(data.new)  # Number of observations

    # drc
    obj.drc.BFGS <- drm(Response ~ Dose,
                     data = data.new,
                     fct = LL.4(names = c("Slope", "Lower limit", "Upper limit", "IC50")))

    result <- coef(obj.drc.BFGS)
    result <- result[c(3, 4, 1, 2)]
    result[3] <- -result[3]
    result <- c(result, obj.drc.BFGS$fit$value/n)

    # drra
    obj.drra <- drra(Response ~ Dose,
                     data = data.new,
                     grad = GradientFunction,
                     method.optim = "BFGS")

    parm.drra <- coef(obj.drra)

    result <- rbind(result, c(parm.drra, obj.drra$error.value))

    row.names(result) <- c("drc", "drra")
    colnames(result) <- c("Lower limit", "IC50", "Slope", "Upper limit", "Loss value")

    print(result)
    cat("\n")

  # When there are more than one drug.
  } else {

    data.new <- subset(x = data.to.comp, select = c(var.ref, var.dose, var.response))
    colnames(data.new) <- c("Ref", "Dose", "Response")
    data.new$Ref <- as.factor(data.new$Ref)
    data.new <- subset(x = data.new, subset = Ref != "999")
    data.new <- droplevels(data.new)

    level.ref <- levels(data.new$Ref)

    for(i in 1:length(level.ref)) {
      data.each <- subset(x = data.new, subset = Ref == level.ref[i])
      n <- nrow(data.each)  # Number of observations

      cat(var.ref, "=", level.ref[i], "\n")

      # drc
      obj.drc.BFGS <- drm(Response ~ Dose,
                          data = data.each,
                          fct = LL.4(names = c("Slope", "Lower limit", "Upper limit", "IC50")))

      result <- coef(obj.drc.BFGS)
      result <- result[c(3, 4, 1, 2)]
      result[3] <- -result[3]
      result <- c(result, obj.drc.BFGS$fit$value/n)

      # drra
      obj.drra <- drra(Response ~ Dose,
                       data = data.each,
                       grad = GradientFunction,
                       method.optim = "BFGS")

      parm.drra <- coef(obj.drra)

      result <- rbind(result, c(parm.drra, obj.drra$error.value))

      row.names(result) <- c("drc", "drra")
      colnames(result) <- c("Lower limit", "IC50", "Slope", "Upper limit", "Loss value")

      print(result)
      cat("\n")
    }
  }
}

# -------------------------------------------------------------------------------
### Compare the parameter estimates
#
sink(file = "parameter_comparison_BFGS.txt")

### acidiq
cat("acidiq\n")

CompareInitializations(data.to.comp = acidiq,
                  var.ref = "pct",
                  var.dose = "dose",
                  var.response = "rgr")

### algae
cat("algae\n")

CompareInitializations(data.to.comp = algae,
                  var.dose = "conc",
                  var.response = "vol")

### etmotc
cat("etmotc\n")

CompareInitializations(data.to.comp = etmotc,
                  var.ref = "pct1",
                  var.dose = "dose1",
                  var.response = "rgr1")

### G.aparine
cat("G.aparine\n")

CompareInitializations(data.to.comp = G.aparine,
                  var.dose = "dose",
                  var.response = "drymatter")

### glymet
cat("glylmet\n")

CompareInitializations(data.to.comp = glymet,
                       var.ref = "pct",
                       var.dose = "dose",
                       var.response = "rgr")

### heartrate
cat("heartrate\n")

CompareInitializations(data.to.comp = heartrate,
                  var.dose = "pressure",
                  var.response = "rate")

### leaflength
cat("leaflength\n")

CompareInitializations(data.to.comp = leaflength,
                  var.dose = "Dose",
                  var.response = "DW")

### lepidium
cat("lepidium\n")

CompareInitializations(data.to.comp = lepidium,
                  var.dose = "conc",
                  var.response = "weight")

### lettuce
cat("lettuce\n")

CompareInitializations(data.to.comp = lettuce,
                  var.dose = "conc",
                  var.response = "weight")

### mecter
cat("mecter\n")

CompareInitializations(data.to.comp = mecter,
                  var.ref = "pct",
                  var.dose = "dose",
                  var.response = "rgr")

### M.bahia
cat("M.bahia\n")

CompareInitializations(data.to.comp = M.bahia,
                  var.dose = "conc",
                  var.response = "dryweight")

### nasturtium
cat("nasturtium\n")

CompareInitializations(data.to.comp = nasturtium,
                  var.dose = "conc",
                  var.response = "weight")

### O.mykiss
cat("O.mykiss\n")

CompareInitializations(data.to.comp = O.mykiss,
                  var.dose = "conc",
                  var.response = "weight")

### P.promelas
cat("P.promelas\n")

CompareInitializations(data.to.comp = P.promelas,
                  var.dose = "conc",
                  var.response = "dryweight")

### ryegrass
cat("ryegrass\n")

CompareInitializations(data.to.comp = ryegrass,
                  var.dose = "conc",
                  var.response = "rootl")

### S.alba
cat("S.alba\n")

CompareInitializations(data.to.comp = S.alba,
                  var.ref = "Herbicide",
                  var.dose = "Dose",
                  var.response = "DryMatter")

### S.capricornutum
cat("S.capricornutum\n")

CompareInitializations(data.to.comp = S.capricornutum,
                  var.dose = "conc",
                  var.response = "count")

### secalonic
cat("secalonic\n")

CompareInitializations(data.to.comp = secalonic,
                  var.dose = "dose",
                  var.response = "rootl")

### spinach
cat("spinach\n")

CompareInitializations(data.to.comp = spinach,
                  var.ref = "CURVE",
                  var.dose = "DOSE",
                  var.response = "SLOPE")

### terbuthylazin
cat("terbuthylazin\n")

CompareInitializations(data.to.comp = terbuthylazin,
                  var.dose = "dose",
                  var.response = "rgr")

### vinclozolin
cat("vinclozolin\n")

CompareInitializations(data.to.comp = vinclozolin,
                  var.ref = "exper",
                  var.dose = "conc",
                  var.response = "effect")

sink()

# -------------------------------------------------------------------------
### Analyze the cases in which the packages differ from each other
#
